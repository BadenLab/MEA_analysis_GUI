function out = Calculate_spatial_kernels_hdf5_app (savepath, add_info)
...
%% Check for frozen noise repeats
locs_begin = locs_diff_norm > 5*mean(locs_diff_norm);
% locs_begin = find(locs_begin)+2;
% locs_begin = downsample(locs_begin,2);

% locs_begin = [1,1];
% locs_begin(3:2+numel(locs_begin_temp)) = locs_begin_temp;
nr_frozen = (nnz(locs_begin)+1);


locs_per_frozen = numel(locs_s)/nr_frozen;
locs_per_frozen = ceil(locs_per_frozen);
repeats_frozen = numel(locs_s)/locs_per_frozen;


%% Load stimulus files --text file version--
...
% UV_name = 'UV_Noise';
% 
% Colour_noise(:,:,1) = importdata([colour_noise_path, '\', Red_name,'.txt'],',');
% Colour_noise(:,:,2) = importdata([colour_noise_path, '\', Green_name,'.txt'],',');
% Colour_noise(:,:,3) = importdata([colour_noise_path, '\', Blue_name,'.txt'],',');
% Colour_noise(:,:,4) = importdata([colour_noise_path, '\', UV_name,'.txt'],',');
% 
% Colour_noise(1,:,:) = [];


%% Load stimulus files --hdf5 version--
%Get the number of boxes used in the noise from user input
...
Colour_noise = zeros(locs_per_frozen*nr_frozen,nr_boxes,4);
a = 1;
b = locs_per_frozen;
for ii = 1:nr_frozen
    
    for kk = 1:4
        Colour_noise(a:b,:,kk) = Colour_noise_temp(:,:,kk);
    end
    a = b+1;
    b = b+locs_per_frozen;
end
...
Kernel_location_temp = strings(stx_rs,2);
