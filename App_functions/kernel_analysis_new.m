function out = kernel_analysis_new(savepath, add_info)
nr_peaks = 2;
stim_idx = add_info.stim_idx;
thr = 4;

for ff = 1:size(add_info.settings.kernel_new.folderplot,2)
    plot_folder = add_info.settings.kernel_new.folderplot{:,ff};
try
    
     S = load(findfile_app(stim_idx,savepath,'RF_overview','subfolder',plot_folder));
     RF_overview = S.RF_overview;
     if contains(add_info.settings.kernel_new.folderplot,'SS_Std')
        test = nnz([RF_overview.STASD_ASP_RF]);
        %Create log array for cells that have passed the QC
        true_kernel_log = [RF_overview.STASD_ASP_RF];
   
     elseif contains(add_info.settings.kernel_new.folderplot,'SC_Std')
        test = nnz([RF_overview.SC_ASP_RF]);
        true_kernel_log = [RF_overview.SC_ASP_RF];
     end
      
     if test == 0
         out = 0; % Return 0 because field doesnt exist 
         disp('No RFs found, set another threshold')
         return 
     end
    ...
    disp('No Dataset selected, select Dataset(s) first')
    out = 0;
    return
end
    ...
    for ii = true_kernel_idx'
       ...
       try
          RF_file = strcat(RF_overview(ii).file,'.bin');
