function RF_Ident = RF_Ident_fn_v5(stimulus_arr,trig_times_vec,spike_times_vec,length_spike_times,p)
% RF Indentification Function - ver. 5
% 29,08,2020 onwards
...
for i = 1:p.Spectral_Dim
    ...
    if p.RF_Ident_Meth_vec(1) == 1
        ...
        if p.STA_Choice  == 2 % subtract average stim
            ...
            if (p.STE_Full_Choice == 2) && (p.RF_Ident_Meth_vec(4) == 1) % No STE_Full and % PAR Mod 27,08,2020 (whole if statement)
               meanSpikeTrigStim(:,:,:,i) = meanSTS_fn(stimulus_arr(:,:,:,i),p,STE); 
            end
        else
    ...
    if p.RF_Ident_Meth_vec(4) == 1
        if p.STE_Full_Choice == 1 % PAR Mod 27,08,2020 --> added if statement
            ...
            [SCA_Stixel_covar(:,:,:,i),SCA_Pixel_covar(:,:,i)] = SC_fn_v4(stimulus_arr(:,:,:,i),STE,meanSpikeTrigStim(:,:,:,i),p); % PAR Mod 27,08,2020
        end
    end
