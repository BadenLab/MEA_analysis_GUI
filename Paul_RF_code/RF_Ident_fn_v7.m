function RF_Ident = RF_Ident_fn_v7(stimulus_arr,trig_times_vec,spike_times_vec,length_spike_times,p)
% RF Indentification Function - ver. 7
% 09,10,2020 onwards
...
if p.Spectral_Dim > 1
    ...
    if     p.RF_Ident_Meth_vec(1) == 1 % STA-SD
        ...
        if     p.RF_Type(1) == 1 % Box
            if (p.Time_Plot_Choice == 2) && (~isempty(RF_Ident.STASD_Box_FullRF_Num_pixels))
        ...
        if p.RF_Type(2) == 1     % All Significant Pixels
            if (p.Time_Plot_Choice == 2) && (~isempty(RF_Ident.STASD_ASP_FullRF_Num_pixels))
                loop_maxSTASD = NaN(p.Spectral_Dim,1);
                for i = 1:p.Spectral_Dim
                    if ~isempty(RF_Ident.STASD_ASP_Num_RF_pixels{i})
    ...
    if     p.RF_Ident_Meth_vec(4) == 1 % SC
        ...
        if     p.RF_Type(1) == 1 % Box
            if (p.Time_Plot_Choice == 2) && (~isempty(RF_Ident.SC_Box_FullRF_Num_pixels))
        ...
        if p.RF_Type(2) == 1     % All Significant Pixels
            if (p.Time_Plot_Choice == 2) && (~isempty(RF_Ident.SC_ASP_FullRF_Num_pixels))
                loop_minSC = NaN(p.Spectral_Dim,1);
                for i = 1:p.Spectral_Dim
                    if ~isempty(RF_Ident.SC_ASP_Num_RF_pixels{i})
    ...
    for i = 1:p.Spectral_Dim
        ...
        if     p.RF_Ident_Meth_vec(1) == 1 % STA-SD
            ...
            if     p.RF_Type(1) == 1 % Box
                ...
                if RF_Ident.STASD_Box_FullRF_Num_pixels
            ...
            if p.RF_Type(2) == 1     % All Significant Pixels
%                 if ~isempty(RF_Ident.STASD_ASP_FullRF_Num_pixels)
                 if RF_Ident.STASD_ASP_FullRF_Num_pixels
            ...
            if p.RF_Type(3) == 1     % Gaussian
%                 if ~isempty(RF_Ident.STASD_Gaus_FullRF_Num_pixels)
                 if RF_Ident.STASD_Gaus_FullRF_Num_pixels
        ...
        if     p.RF_Ident_Meth_vec(4) == 1 % SC
            ...
            if     p.RF_Type(1) == 1 % Box
                %if ~isempty(RF_Ident.SC_Box_FullRF_Num_pixels)
                if RF_Ident.SC_Box_FullRF_Num_pixels
            ...
            if p.RF_Type(2) == 1     % All Significant Pixels
                %if ~isempty(RF_Ident.SC_ASP_FullRF_Num_pixels)
                if RF_Ident.SC_ASP_FullRF_Num_pixels
            ...
            if p.RF_Type(3) == 1     % Gaussian
                %if ~isempty(RF_Ident.SC_Gaus_FullRF_Num_pixels)
                if RF_Ident.SC_Gaus_FullRF_Num_pixels
