function [RF_coords,Num_RF_rows,Num_RF_cols,Num_RF_pixels] = Box_RF_fn_alt_v3(Centre_RF_coords,p)
%%% Find the box RF coordinates & stats
% Modified from v2 on 18,12,2020
%
% Input:
...
if max_RF_row_temp + p.RF_layers <= p.stim_rows - 1    % Mod 18,12,2020 (was - p.RF_layers)
    max_RF_row = max_RF_row_temp + p.RF_layers;
else
    max_RF_row = p.stim_rows;
end

if max_RF_col_temp + p.RF_layers <= p.stim_columns - 1 % Mod 18,12,2020 (was - p.RF_layers)
...
RF_coords = NaN(Num_RF_rows*Num_RF_cols,2); % Mod 18,12,2020 (was - p.RF_layers)
